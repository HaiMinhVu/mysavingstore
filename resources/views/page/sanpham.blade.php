	...
	<div id="content">
		<div class="row">
			<div class="col-sm-9">
				<div class="row">
					<div class="col-sm-4">
						<a href="upload/product/{{$product->image}}" target="_blank"><img src="upload/product/{{$product->image}}" alt="{{$product->name}}"></a>
					</div>
					<div class="col-sm-8">
						<div class="single-item-body">
							<p class="single-item-title pro-title">{{$product->name}}</p>
							@if($product->promotion_price == 0)
								<p class="single-item-price"><span>${{$product->unit_price}}</span></p>
							@else
							...
							<div class="single-item">
								...
								<div class="single-item-body">
									...
									@if($row_same->promotion_price > 0)
										<p class="single-item-price">
											<span class="flash-del">${{$row_same->unit_price}}</span>
											<span class="flash-sale">${{$row_same->promotion_price}}</span>
										</p>
									@else
										<p class="single-item-price"><span>${{$row_same->unit_price}}</span></p>
									@endif
						...
						@foreach($best_seller as $row_seller)
							<div class="media beta-sales-item">
								...
								<div class="media-body">
									...
									@if($row_seller->promotion_price == 0.00)
										<div class="beta-sales-price">${{$row_seller->unit_price}}</div>
									@else
										<div class="beta-unit-price">${{$row_seller->unit_price}}</div>
										<div class="beta-sales-price">${{$row_seller->promotion_price}}</div>
									@endif
								</div>
							</div>
						@endforeach
							...
							<div class="media beta-sales-item">
								...
								<div class="media-body">
									...
									@if($row_new->promotion_price == 0)
										<div class="beta-sales-price">${{$row_new->unit_price}}</div>
									@endif
									@if($row_new->promotion_price > 0)
										<div class="beta-unit-price">${{$row_new->unit_price}}</div>
										<div class="beta-sales-price">${{$row_new->promotion_price}}</div>
									@endif
								</div>
							</div>
						@endforeach
